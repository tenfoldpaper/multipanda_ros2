...
public:
    GarmiHeadSim() = delete;
    GarmiHeadSim(std::string& robot_name) : robot_name_(robot_name){}
    ~GarmiHeadSim();
    bool populateIndices(mjModel* m_);

private:
    std::string robot_name_;
    std::array<int, 1UL> link_indices_; // for nbody indexing
    ...
    std::array<int, 2UL> act_trq_indices_; // part of nu

    // bool setModelIndices();
    const int kNumberOfJoints = 2;
    const std::array<std::string, 2UL> head_joints_{"head_pan","head_tilt"};
