...
#include "franka_hardware/garmi_mujoco_hardware_interface.hpp"
...
CallbackReturn GarmiMujocoHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
  ...
  if (info_.joints.size() != kNumberOfJoints * robot_count_ + 4) { // base = 2, head = 2
  ...
  for (const auto& joint : info_.joints) {
    ...
    if(joint.name.find("garmi_base") != std::string::npos){ // skip the wheel joints
      continue;
    }

    if(joint.name.find("garmi_head") != std::string::npos){ // skip the head joints
      continue;
    }
  ...
  // mobile base
  mobile_base_ = MobileBaseContainer();
  std::string base_name = "garmi_base";
  mobile_base_.robot_name_ = base_name;
  mobile_base_.base_ = std::make_unique<MobileBaseSim>(base_name);
  mobile_base_.base_->populateIndices(m_);
  mobile_base_.hw_commands_joint_velocity_.fill(0);
  mobile_base_.hw_velocities_.fill(0);

  // garmi head
  garmi_head_ = GarmiHeadContainer();
  std::string head_name = "head";
  garmi_head_.robot_name_ = head_name;
  garmi_head_.head_ = std::make_unique<GarmiHeadSim>(head_name);
  garmi_head_.head_->populateIndices(m_);
  garmi_head_.hw_commands_joint_effort_.fill(0);
  garmi_head_.hw_efforts_.fill(0);
  garmi_head_.hw_positions_.fill(0);
  garmi_head_.hw_velocities_.fill(0);
  ...
  if(mj_yaml_path.length() != 0){
    ...
    try{
      ...
      if(!mj_object_names.empty()){
        ...
        std::chrono::milliseconds(33), std::bind(&GarmiMujocoHardwareInterface::publishObjectContainers, this));
  ...
  for(auto &arm_p : arms_){
    for(size_t i = 0; i<kNumberOfJoints; i++){
      ...
      if(arm_p.first == "left" && i == 0){
        d_->qpos[arm_p.second.robot_->joint_qpos_indices_[i]] *= -1;
      }
    }
  }
...
std::vector<StateInterface> GarmiMujocoHardwareInterface::export_state_interfaces() {
  std::vector<StateInterface> state_interfaces;

  for (auto i = 0U; i < info_.joints.size(); i++) {
    if(info_.joints[i].name.find("garmi_base") != std::string::npos){
      int wheel = 0;
      if(info_.joints[i].name.find("right") != std::string::npos){
        wheel = 1;
      }
      state_interfaces.emplace_back(StateInterface(
        info_.joints[i].name, hardware_interface::HW_IF_POSITION, &mobile_base_.hw_positions_.at(wheel)));
      state_interfaces.emplace_back(StateInterface(
        info_.joints[i].name, hardware_interface::HW_IF_VELOCITY,  &mobile_base_.hw_velocities_.at(wheel)));
      continue;
    }
    else if(info_.joints[i].name.find("garmi_head") != std::string::npos){
      int head = 0;
      if(info_.joints[i].name.find("2") != std::string::npos){
        head = 1;
      }
      std::cout << "Export state: Got head " << head << std::endl;
      state_interfaces.emplace_back(StateInterface(
        info_.joints[i].name, hardware_interface::HW_IF_POSITION, &garmi_head_.hw_positions_.at(head)));
      state_interfaces.emplace_back(StateInterface(
        info_.joints[i].name, hardware_interface::HW_IF_VELOCITY,  &garmi_head_.hw_velocities_.at(head)));
      state_interfaces.emplace_back(StateInterface(
        info_.joints[i].name, hardware_interface::HW_IF_EFFORT,  &garmi_head_.hw_efforts_.at(head)));
      continue;
    }
    state_interfaces.emplace_back(StateInterface(
...
std::vector<CommandInterface> GarmiMujocoHardwareInterface::export_command_interfaces() {
  ...
  for (auto i = 0U; i < info_.joints.size(); i++) {
    if(info_.joints[i].name.find("garmi_base") != std::string::npos){
      ...
      command_interfaces.emplace_back(CommandInterface( // JOINT VELOCITY
        info_.joints[i].name, hardware_interface::HW_IF_VELOCITY, &mobile_base_.hw_commands_joint_velocity_.at(wheel)));
      continue;
    }
    if(info_.joints[i].name.find("garmi_head") != std::string::npos){
      int head = 0;
      if(info_.joints[i].name.find("2") != std::string::npos){
        head = 1;
      }
      std::cout << "Export command: Got head " << head << std::endl;
      command_interfaces.emplace_back(CommandInterface( // JOINT EFFORT
        info_.joints[i].name, hardware_interface::HW_IF_EFFORT, &garmi_head_.hw_commands_joint_effort_.at(head)));
      continue;
    }
    command_interfaces.emplace_back(CommandInterface( // JOINT EFFORT
...
CallbackReturn GarmiMujocoHardwareInterface::on_activate(
...
CallbackReturn GarmiMujocoHardwareInterface::on_deactivate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
...
hardware_interface::return_type GarmiMujocoHardwareInterface::read(const rclcpp::Time& /*time*/,
  ...
  // base
  mobile_base_.hw_positions_ = std::array<double, 2UL>{d_->qpos[mobile_base_.base_->joint_qpos_indices_[0]], d_->qpos[mobile_base_.base_->joint_qpos_indices_[1]]};
  mobile_base_.hw_velocities_ = std::array<double, 2UL>{d_->qvel[mobile_base_.base_->joint_qvel_indices_[0]], d_->qvel[mobile_base_.base_->joint_qvel_indices_[1]]};
  
  // head
  garmi_head_.hw_efforts_ = std::array<double, 2UL>{d_->actuator_force[garmi_head_.head_->act_trq_indices_[0]], d_->actuator_force[garmi_head_.head_->act_trq_indices_[1]]};
  garmi_head_.hw_positions_ = std::array<double, 2UL>{d_->qpos[garmi_head_.head_->joint_qpos_indices_[0]], d_->qpos[garmi_head_.head_->joint_qpos_indices_[1]]};
  garmi_head_.hw_velocities_ = std::array<double, 2UL>{d_->qvel[garmi_head_.head_->joint_qvel_indices_[0]], d_->qvel[garmi_head_.head_->joint_qvel_indices_[1]]};
...
hardware_interface::return_type GarmiMujocoHardwareInterface::write(const rclcpp::Time& /*time*/,
                                                               const rclcpp::Duration& /*period*/) {
  std::lock_guard<std::mutex> guard(mj_mutex_);
  for(auto& arm_container_pair: arms_){
    auto &arm = arm_container_pair.second;
  ...
  // base control
  d_->ctrl[mobile_base_.base_->act_vel_indices_[0]] = mobile_base_.hw_commands_joint_velocity_[0];
  d_->ctrl[mobile_base_.base_->act_vel_indices_[1]] = mobile_base_.hw_commands_joint_velocity_[1];

  // head control 
  d_->ctrl[garmi_head_.head_->act_trq_indices_[0]] = garmi_head_.hw_commands_joint_effort_[0];
  d_->ctrl[garmi_head_.head_->act_trq_indices_[1]] = garmi_head_.hw_commands_joint_effort_[1];
...
hardware_interface::return_type GarmiMujocoHardwareInterface::prepare_command_mode_switch(
  ...
  // //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
  // Query if the interface has to do with base or head
  std::vector<std::string> base_stop_interfaces;
  std::vector<std::string> head_stop_interfaces;
  for(auto& stop : stop_interfaces){
    if(stop.find("garmi_base") != std::string::npos){
      base_stop_interfaces.push_back(stop);
    }
    else if(stop.find("garmi_head") != std::string::npos){
      head_stop_interfaces.push_back(stop);
    }
  }
  if(base_stop_interfaces.size() > 0){
    if(base_stop_interfaces.size() != 2){
      RCLCPP_ERROR(this->getLogger(), "The requested base stop interface's size is not 2!\n"\
                                        "Please Make sure you control both wheel joints.");
      return hardware_interface::return_type::ERROR;
    }
    if(!all_of_element_has_string(base_stop_interfaces, "velocity")){
      RCLCPP_ERROR(this->getLogger(), "The requested base stop interface is not of type velocity.\n"\
                                        "Only velocity command is supported for base.");
      return hardware_interface::return_type::ERROR;                                
    }
    // now we flag it for changing.
    mobile_base_.switch_cm_ = true;
    mobile_base_.control_mode_ = ControlMode::None;
  }
  ...
  for(auto& arm : arms_){
    ...
    for(auto& stop : stop_interfaces){
      if(stop.find(arm.first) != std::string::npos && stop.find("garmi_base") == std::string::npos && stop.find("garmi_head") == std::string::npos){
    ...
    switch(stop_type){
      ...
      case 0: // if size is 0, move on to the next arm
        RCLCPP_INFO(this->getLogger(), "Requested stop interface is not for the arms.");
        continue;
        break;
  ...
  // //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
  // Query if the interface has to do with base or head
  std::vector<std::string> base_start_interfaces;
  std::vector<std::string> head_start_interfaces;
  for(auto& stop : start_interfaces){
    if(stop.find("garmi_base") != std::string::npos){
      base_start_interfaces.push_back(stop);
    }
    else if(stop.find("garmi_head") != std::string::npos){
      head_start_interfaces.push_back(stop);
    }
  }
  if(base_start_interfaces.size() > 0){
    if(base_start_interfaces.size() != 2){
      RCLCPP_ERROR(this->getLogger(), "The requested base start interface's size is not 2!\n"\
                                        "Please Make sure you control both wheel joints.");
      return hardware_interface::return_type::ERROR;
    }
    if(!all_of_element_has_string(base_start_interfaces, "velocity")){
      RCLCPP_ERROR(this->getLogger(), "The requested base start interface is not of type velocity.\n"\
                                        "Only velocity command is supported for base.");
      return hardware_interface::return_type::ERROR;                                
    }
    // now we flag it for changing.
    mobile_base_.switch_cm_ = true;
    mobile_base_.control_mode_ = ControlMode::JointVelocity;
  }
  ...
  for(auto& arm : arms_){
    ...
    for(auto& start : start_interfaces){
      if(start.find(arm.first) != std::string::npos && start.find("base") == std::string::npos && start.find("head") == std::string::npos){
    ...
    switch(start_type){
      case 0: // if size is 0, move on to the next arm
        RCLCPP_INFO(this->getLogger(), "Requested start interface is not for the arms.");
        continue;
        break;
...
hardware_interface::return_type GarmiMujocoHardwareInterface::perform_command_mode_switch(
    const std::vector<std::string>& /*start_interfaces*/,
    const std::vector<std::string>& /*stop_interfaces*/) {

  RCLCPP_INFO(this->getLogger(),"Performing command mode switch");

  if(mobile_base_.switch_cm_){
    if(mobile_base_.control_mode_ == ControlMode::None){
        set_velocity_servo(m_, mobile_base_.base_->act_vel_indices_[0], 0);
        set_velocity_servo(m_, mobile_base_.base_->act_vel_indices_[1], 0);
    }
    else if(mobile_base_.control_mode_ == ControlMode::JointVelocity){
        set_velocity_servo(m_, mobile_base_.base_->act_vel_indices_[0], 5);
        set_velocity_servo(m_, mobile_base_.base_->act_vel_indices_[1], 5);
    }
    RCLCPP_INFO_STREAM(this->getLogger(), "Base " << mobile_base_.robot_name_ << 
                                            " current mode: " << mobile_base_.control_mode_);
    mobile_base_.switch_cm_ = false;
  }
  for(auto &arm_p : arms_){
...
bool GarmiMujocoHardwareInterface::populateObjectContainers(std::vector<std::string> obj_names){
...
bool GarmiMujocoHardwareInterface::updateObjectContainer(std::string obj_name){
  auto &mj_obj = mj_objs_[obj_name];
...
void GarmiMujocoHardwareInterface::changeMjObjPose(std::string obj_name, double x, double y, double z, double qx, double qy, double qz, double qw){
...
void GarmiMujocoHardwareInterface::changeMjBodyPose(std::string obj_name, double x, double y, double z, double qx, double qy, double qz, double qw){
  auto &mj_obj = mj_objs_[obj_name];
...
void GarmiMujocoHardwareInterface::changeMjMocapPose(std::string obj_name, double x, double y, double z, double qx, double qy, double qz, double qw){
  auto &mj_obj = mj_objs_[obj_name];
...
void GarmiMujocoHardwareInterface::publishObjectContainers(){
...
rclcpp::Logger GarmiMujocoHardwareInterface::getLogger() {
  return rclcpp::get_logger("GarmiMujocoHardwareInterface");
}
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(franka_hardware::GarmiMujocoHardwareInterface,
